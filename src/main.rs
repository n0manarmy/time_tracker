...
fn build(application: &gtk::Application) {
    ...
    // let tx1 = glib::MainContext::Sender::clone(&tx);
    // let (log_sender, log_receiver) = glib::MainContext::channel(glib::PRIORITY_DEFAULT);
    ...
    // button_time_in.connect_clicked(move |_| {
    //     let time_stamp = TimeUtils::get_time_stamp("IN");
    //     &tx.send(time_stamp).unwrap();
    //     // match &tx.send(time_stamp) {
    //     //     Ok(ok) => ok,
    //     //     Err(why) => panic!("{}", why),
    //     // }
    // });

    // button_time_out.connect_clicked(move |_| {
    //     let time_stamp = TimeUtils::get_time_stamp("OUT");
    //     &tx1.send(time_stamp).unwrap();
    // });
